...
mysqlConnection.connect((err) => {
  if (err) {
    ...
    console.log(chalk.bgRed("Connected to MySQL successfully"));
...
async function handleButtonInteraction(interaction, client) {
  ...
  if (customId === "open_form") {
    ...
    switch (action) {
      case "approve":
        console.log(
          chalk.red(`Approving submission for userId: ${userIdForAction}`)
        );
        db.get(
          `SELECT userId FROM user_submissions WHERE userId = ?`,
          [userIdForAction],
      ...
      case "approve_gaixinh":
        console.log(
          chalk.red(`Approving 'Gái Xinh' role for userId: ${userIdForAction}`)
        );
        db.get(
          `SELECT userId FROM user_submissions WHERE userId = ?`,
          [userIdForAction],
      ...
      case "approve_bacsi":
        console.log(
          chalk.red(`Approving 'Bác sĩ' role for userId: ${userIdForAction}`)
        );
        db.get(
          `SELECT userId FROM user_submissions WHERE userId = ?`,
          [userIdForAction],
      ...
      case "approve_police":
        console.log(
          chalk.red(`Approving 'Police' role for userId: ${userIdForAction}`)
        );
        db.get(
          `SELECT userId FROM user_submissions WHERE userId = ?`,
          [userIdForAction],
      ...
      case "approve_mechanic":
        console.log(
          chalk.red(`Approving 'Cứu Hộ' role for userId: ${userIdForAction}`)
        );
        db.get(
          `SELECT userId FROM user_submissions WHERE userId = ?`,
          [userIdForAction],
...
async function handleModalSubmit(interaction, client) {
  if (interaction.customId === "personal_info_form") {
    ...
    const vnTimeString = vnTime.toLocaleString("vi-VN", {
    ...
    db.run(insertSql, insertValues, async (err) => {
      ...
      const infoEmbed = new EmbedBuilder()
        ...
        .setTitle("Thông Tin đăng kí whitelist")
        .setThumbnail(
          "https://cdn.discordapp.com/attachments/1229486110695100507/1229545379427651665/Untitled_design_8.gif?ex=6634af9e&is=66335e1e&hm=c3873fbf908bc99a6a0ef6052638ed91cd19dfc0c602cab2bcffd8582df6f209&"
        )
        .addFields(
    ...
    db.get(
      ...
      async (err, row) => {
        ...
        const rejectionEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `Quản trị viên <@${interaction.user.id}> đã từ chối form của người dùng <@${submittedUserId}> với lý do: \`\`\`\n${rejectReason}\n\`\`\``
          )
          .setTimestamp();
